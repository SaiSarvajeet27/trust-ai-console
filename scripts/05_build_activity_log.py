...
followed, plus a filtered subset (security actions only) so the designers can
fake a "search/filter" state in Figma.
...
ENTRIES = [
    ...
    ActivityEntry("LOG-013", "2026-06-18 16:02", "Roll out patch KB5031900",
                  ...
                  "Approved", "A. Sharma", "Rolled out with no failures."),
    ...
    ActivityEntry("LOG-011", "2026-06-17 14:55", "Block external USB on Finance group",
                  ...
                  "Overridden", "M. Okafor", "Admin kept USB enabled for two approved users."),
    ActivityEntry("LOG-010", "2026-06-17 10:08", "Isolate device DSK-2204",
                  "1 device", "Flagged unusual traffic and recommended isolation.",
                  "Escalated", "M. Okafor", "Sent to the security team for review."),
    ...
    ActivityEntry("LOG-008", "2026-06-16 09:12", "Patch rollback on WKS-1502",
                  ...
                  "Approved", "R. Costa", "Device recovered after rollback."),
...
def main() -> None:
    ...
    filtered = [e for e in all_entries
                if any(k in e["action"].lower()
                       for k in ("quarantine", "isolate", "firewall", "re-auth"))]
